...
var FSHADER_SOURCE = `
  ...
  void main() {
    if (u_whichTexture == -3) {
      ...
      gl_FragColor = vec4(1, 1, 1, 1); // error, use white
    }
    ...
    if (u_lightOn) {
      ...
      // modify light color based on a slider
    ...
    if (u_spotlightOn) {
      ...
      vec3 L2 = normalize(u_spotlightPos - vec3(v_VertPos));
      float spotCosine = dot(spotDirection, L2);
      if (spotCosine >= u_spotCosineCutoff) {
        spotFactor = pow(spotCosine, u_spotExponent);
        spotFactor = spotFactor + 1.0;
      } else {
        spotFactor = 1.0;
      }
...
let g_spotCosineCutoff = 0.9;
let g_spotExponent = 64;
...
function HTML_UI_functions() {
  ...
  document.getElementById('lightColorR').addEventListener("mousemove", function(ev) { if (ev.buttons == 1) { g_lightColor[0] = this.value; renderAllShapes(); } });
  document.getElementById('lightColorG').addEventListener("mousemove", function(ev) { if (ev.buttons == 1) { g_lightColor[1] = this.value; renderAllShapes(); } });
  document.getElementById('lightColorB').addEventListener("mousemove", function(ev) { if (ev.buttons == 1) { g_lightColor[2] = this.value; renderAllShapes(); } });
...
function updateAnimationAngles() {
  ...
  g_spotlightPos[0] = 1 - Math.cos(g_seconds);
  g_spotlightPos[2] = 1 - Math.sin(g_seconds) - 1;
...
function renderAllShapes() {
  ...
  gl.enable(gl.DEPTH_TEST);
    ...
    // draw the sky
    // Make it smaller to test out the lighting
    var sky = new Cube();
    sky.textureNum = 1;
    if (g_normalOn) {
      sky.textureNum = -3;
    ...
    sphere.matrix.translate(1.4, 2.0, 0.7);
    sphere.render();
